...
int main(){
    ...
    for(int i = 1; i <= 100; i++){ //100번의 반복으로 1%~100% 까지 구현
       while (count <= 10000000){  //10억번 반복하기위해 100*1000만번의 실행
            x = r * ((double)rand() / (double)RAND_MAX); //x의 값을 0~1 사이의 실수로 랜덤하게 지정
            y = r * ((double)rand() / (double)RAND_MAX); //y의 값을 0~1 사이의 실수로 랜덤하게 지정
            if (sqrt(pow(x, 2) + pow(y, 2)) < r){ //sqrt(pow(x, 2)+(pow(y, 2)) < 1)을 만족하면 원 안에 있는 점
                circle++; //원안에 있는 점 을 의미하는 변수를 +1 함
            }
            count ++;
            cnt ++; //count 변수가 1000만번마다 초기화 되므로 새로운 카운트 값을 1씩 증가
         }
        count = 1; //1000만번을 100번 돌리므로 1000만번의 while문 실행 후 count 값을 0 으로 초기화
